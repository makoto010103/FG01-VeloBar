...
void loop() {
  ...
  if (IMU.getAGT() > 0) {
    ...
    if (is_static) {
        ...
        // --- ② 加速度連動型リーキー積分 (v4.2) ---
        // 加速中：信号保存 (0.9995) / 減速中：高速減衰 (0.995)
        // 0.9995 @ 200Hz = 90%保持/秒 (レップ中の加速を守る)
        // 0.995  @ 200Hz = 37%保持/秒 (レップ後の減衰を2-3秒で完了)
        
        if (velocity < 0.0f) {
            // 【下降・反射領域】
            velocity *= 0.90f; 
        } else if (corrected_accel < -0.3f) {
            // 【減速フェーズ】ピークを過ぎて重力に負けている
            // ピーク後の速度が張り付く問題(0.8m/sが5秒以上続く)を根絶
            velocity *= 0.995f;
        } else {
            // 【真の挙上フェーズ（粘りゾーン含む）】
            velocity *= 0.9995f; 
